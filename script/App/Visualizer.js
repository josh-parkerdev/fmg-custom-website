System.Visualizer = function(app) {
	this.app = app;
	var self = this;

	this.data = System.visualizer;

	this.$app = $('<div>', {id: 'visualizer-container', class: 'embedded'});
	$(this.$app).css('zIndex', -1);

	$(this.$app).load('visualizer/index.html', function() {
 ...
 */
var Visualizer = function(cWidth, cHeight) {
	var tileSize;
	var tiles = [];
	var stars = [];
	// canvas vars
	...
	this.resizeCanvas = function() {
		var cWidth = System.visualizer.css.width;
		var cHeight = System.visualizer.css.height;
		var sx = FutureShip.scale[0];
		var sy = FutureShip.scale[1];

		if (fgCanvas) {
			// resize the foreground canvas
			fgCanvas.width = cWidth * sx;
			fgCanvas.height = cHeight * sy;
			fgCtx.translate(fgCanvas.width/2,fgCanvas.height/1.5);

			// resize the bg canvas
			bgCanvas.width = cWidth * sx;
			bgCanvas.height = cHeight * sy;
			// resize the starfield canvas
			sfCanvas.width = cWidth * sx;
			sfCanvas.height = cHeight * sy;
			sfCtx.translate(fgCanvas.width/2,fgCanvas.height/1.5);
	...
	this.init = function(options) {
		...
		sfCanvas.style.zIndex = "100";
		sfCtx = sfCanvas.getContext("2d");
		sfCanvas.setAttribute('style', 'position: absolute; z-index: -2');
		container.appendChild(sfCanvas);

		// background image layer
		bgCanvas = document.createElement('canvas');
		bgCanvas.style.zIndex = "100";
		bgCtx = bgCanvas.getContext("2d");
		//sfCanvas.setAttribute('style', 'z-index: -3')
...
System.Visualizer.prototype = {
	...
	setBounds: function() {
		...
		for (property in css) {
			var value = css[property] * scale[0];
	...
	initialize: function() {
		...
		console.log("Visualizer onLoad");
		var visualizer = new Visualizer(width, height);
